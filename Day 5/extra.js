...
const findAnswer = () => {
    ...
    for (let i = 0; i < Infinity && answer.indexOf('*') !== -1; i++) {
        const hash = md5(ROOM_ID + i);

        if (hash.slice(0, 5) === '00000') {
            const offset = parseInt(hash[5], 10);

            if (offset >= 0 && offset <= 7 && answer[offset] === '*') {
                answer = answer.slice(0, offset) + hash[6] + answer.slice(offset + 1);
                console.log(answer);
            }
        }
    }
